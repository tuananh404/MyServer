...
// GeneratedConnection is installed into an APK. Keep the public bootstrap
// identifier out of a plain DEX string table. This is deliberately only an
// obfuscation layer: product identifiers are public and authorization remains
// enforced by the HTTPS API and short-lived session policy.
function javaObfuscatedString(value) {
  const input = Buffer.from(String(value), 'utf8');
  const seed = crypto.createHash('sha256').update(input).digest()[0] || 0x5b;
  const encoded = Array.from(input, (byte, index) =>
    byte ^ ((seed + index * 29 + Math.floor(index / 2) * 7) & 0xff));
  const values = encoded.map(byte => `(byte) 0x${byte.toString(16).padStart(2, '0')}`).join(', ');
  return `decode(${seed}, new byte[] { ${values} })`;
}

function readSdkSourceEntries() {
  ...
  function visit(directory) {
    ...
    for (const child of children) {
      ...
      if (child.isSymbolicLink()) throw new Error('SDK package cannot contain symbolic links.');
      if (child.isDirectory()) {
        visit(absolutePath);
        continue;
      }
      if (!child.isFile()) continue;
      const relativePath = path.relative(sdkRoot, absolutePath).split(path.sep).join('/');
      if (!relativePath || relativePath.startsWith('../')) throw new Error('SDK file escaped its package root.');
      const data = fs.readFileSync(absolutePath);
      totalBytes += data.length;
      if (entries.length >= 200 || totalBytes > 4 * 1024 * 1024) {
        throw new Error('SDK package exceeded the safe serverless bundle limit.');
      }
      entries.push({ name: `serverkey-android-sdk/${relativePath}`, data });
    }
  }

  visit(sdkRoot);
  if (entries.length === 0) throw new Error('ServerKey Android SDK source files were not found.');
  return entries;
}

function buildSdkPackage(manifest) {
  if (!manifest?.connection_uri || !manifest?.project?.project_id || !manifest?.project?.app_version) {
    throw new Error('A complete integration manifest is required to build the SDK package.');
  }
  const projectId = manifest.project.project_id;
  const appVersion = manifest.project.app_version;
  const generatedConnection = `package com.serverkey.sdk;

import java.nio.charset.StandardCharsets;

/** Generated by the authenticated ServerKey dashboard. */
public final class GeneratedConnection {
    public static final String CONNECTION_URI = ${javaObfuscatedString(manifest.connection_uri)};
    public static final String APP_VERSION = ${javaStringLiteral(appVersion)};
    public static final String PROJECT_ID = ${javaStringLiteral(projectId)};

    private static volatile int decodeGuard;

    private static String decode(int seed, byte[] encoded) {
        byte[] decoded = new byte[encoded.length];
        for (int index = 0; index < encoded.length; index++) {
            int mask = seed + index * 29 + (index / 2) * 7 + decodeGuard;
            decoded[index] = (byte) (encoded[index] ^ mask);
        }
...
`;
  const quickStart = `# Generated ServerKey Android SDK V2.1.2
...
Extract the ZIP, then run from the extracted folder:
...
`;
  ...
  entries.push(
    {
      name: 'serverkey-android-sdk/java/com/serverkey/sdk/GeneratedConnection.java',
      data: generatedConnection
    },
    {
      name: 'serverkey-android-sdk/serverkey-manifest.json',
      data: `${JSON.stringify(manifest, null, 2)}\n`
    },
    {
      name: 'serverkey-android-sdk/QUICK_START_GENERATED.md',
      data: quickStart
    }
  );
  const safeProject = projectId.replace(/[^A-Za-z0-9._-]/g, '-');
  const safeVersion = appVersion.replace(/[^A-Za-z0-9._+-]/g, '-');
  return {
    buffer: createStoredZip(entries),
    filename: `serverkey-${safeProject}-${safeVersion}.zip`,
...
app.locals.buildSdkPackage = buildSdkPackage;
...
app.post('/api/admin/sdk-package', verifyAdmin, async (req, res) => {
  const productToken = sanitizeString(req.body.product_token, 128);
  const projectId = sanitizeString(req.body.project_id, 64);
  const appVersion = sanitizeString(req.body.app_version, 32);
  const validationError = validateIntegrationInput(productToken, projectId, appVersion);
  ...
  try {
    ...
    res.set({
      ...
      'Content-Disposition': `attachment; filename="${sdkPackage.filename}"`,
      'Content-Length': String(sdkPackage.buffer.length),
      'X-ServerKey-Package-Files': String(sdkPackage.fileCount),
      'X-Content-Type-Options': 'nosniff',
      'Cache-Control': 'no-store, max-age=0'
    });
    return res.send(sdkPackage.buffer);
  } catch (error) {
    console.error('[SDK PACKAGE] Failed:', error);
    return res.status(500).json({ success: false, message: 'Could not build the private SDK package.' });
  }
});
